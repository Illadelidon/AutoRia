...
namespace AutoRia.Infrastructure.Repository
{
    internal class Repository<TEntity> : IRepository<TEntity> where TEntity : class, IEntity
    {
        ...
        public async Task<IEnumerable<TEntity>> GetAll(object id)
        {
        ...
        public async Task InsertImg(TEntity entity)
        {
            await dbSet.AddAsync(entity);
        }
        public List<ImageDto> GetImagesForPost(int postId)
        {
            var postImages=context.Posts
                .Where(p=>p.Id==postId)
                .SelectMany(p=>p.Imgs)
